...
\subsubsection{Comparación en Data Science (Modelado ML) (***PLANNED***)}

\paragraph{Métricas de entrenamiento del modelo.}

\textit{Esta sección documentará la comparación de performance entre clusters durante el entrenamiento del modelo de regresión lineal con PySpark MLlib una vez implementado.}

\begin{table}[H]
  \centering
  \caption{Comparativa de tiempos de entrenamiento ML (***PLANNED***).}
  ...
  \begin{tabular}{@{}lccc@{}}
    ...
    Etapa & Cluster 1 & Cluster 2 & Mejora \\
    \midrule
    Carga de datos Gold & [tiempo] & [tiempo] & [\%] \\
    Preparación de features & [tiempo] & [tiempo] & [\%] \\
    Entrenamiento del modelo & [tiempo] & [tiempo] & [\%] \\
    Evaluación y métricas & [tiempo] & [tiempo] & [\%] \\
    \midrule
    \textbf{Total ML Phase} & \textbf{[tiempo]} & \textbf{[tiempo]} & \textbf{[\%]} \\
    \bottomrule
  \end{tabular}
\end{table}

\paragraph{Métricas de calidad del modelo.}

\begin{table}[H]
  \centering
  \caption{Comparativa de métricas de evaluación del modelo (***PLANNED***).}
  \label{tab:model-metrics-comparison}
  \scriptsize
  \begin{tabular}{@{}lcc@{}}
    \toprule
    Métrica & Cluster 1 & Cluster 2 \\
    \midrule
    R² (coeficiente de determinación) & [valor] & [valor] \\
    RMSE (error cuadrático medio) & [valor] & [valor] \\
    MAE (error absoluto medio) & [valor] & [valor] \\
    \bottomrule
  \end{tabular}
\end{table}
...
\textit{Se evaluará cómo la configuración del cluster afecta:}

\begin{itemize}
  \item \textbf{Convergencia del modelo:} Tiempo hasta alcanzar convergencia en el algoritmo de optimización
  \item \textbf{Iteraciones por segundo:} Throughput de iteraciones durante el entrenamiento
  \item \textbf{Utilización de memoria:} Impacto de la memoria disponible en el cacheo de datos de entrenamiento
  \item \textbf{Paralelización de cross-validation:} Beneficio de mayor número de workers en validación cruzada
\end{itemize}
...
\begin{enumerate}
  ...
  \item \textbf{[ESPACIO PARA VENTAJA ADICIONAL SOBRE MODELO ML]}
\end{enumerate}
...
\begin{enumerate}
  ...
  \item \textbf{[ESPACIO PARA LIMITACIÓN ADICIONAL SOBRE MODELO ML]}
\end{enumerate}
